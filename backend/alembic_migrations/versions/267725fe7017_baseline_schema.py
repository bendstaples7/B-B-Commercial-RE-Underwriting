...
def upgrade():
    # ------------------------------------------------------------------
    # Bootstrap: create the original schema if it doesn't already exist.
    #
    # This migration was originally written to run against a database that was
    # pre-seeded by migrations/001_create_schema.sql.  To make this migration
    # self-sufficient (no external seed needed), we create all original types
    # and tables here using IF NOT EXISTS / EXCEPTION WHEN duplicate_object.
    #
    # On production databases that were seeded manually, these statements are
    # no-ops (the objects already exist).  On a fresh CI database, they create
    # the schema from scratch, then the ALTER TABLE / ALTER COLUMN blocks below
    # bring it up to the state Alembic expects.
    # ------------------------------------------------------------------

    # Original enum types (the old names — the ALTERs below will rename them)
    op.execute("""
        DO $$ BEGIN
    ...
    # Original tables (all with IF NOT EXISTS)
    op.execute("""
    ...
    # Original indexes (all IF NOT EXISTS)
    ...
    # ------------------------------------------------------------------
    # End bootstrap — everything below is the original baseline migration
    # that renames enum types and normalises column types.
    # ------------------------------------------------------------------
