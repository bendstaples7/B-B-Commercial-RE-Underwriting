...
class TestRealChainValidator:
    ...
    def test_real_chain_head_is_squash_marker(self):
        """The single head must be the d6e7f8a9b0c1 hubspot_signals dedup unique-index revision."""
        from app.migration_utils import assert_single_head_and_root
        result = assert_single_head_and_root()
        assert result["head_revisions"] == ["d6e7f8a9b0c1"], (
            f"Expected head revision 'd6e7f8a9b0c1', got {result['head_revisions']}"
        )
...
class TestCheckMigrationChainScript:
    ...
    def test_script_prints_head_and_root_info_on_success(self):
        ...
        assert "d6e7f8a9b0c1" in output, (
            f"Expected head 'd6e7f8a9b0c1' in script output, got: {output!r}"
        )
