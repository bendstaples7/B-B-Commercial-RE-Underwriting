...
class TestRecentSaleMailReconciliation:
    ...
    def test_activates_skip_trace_only_when_scheduled_date_arrives(self, app):
        ...
        with app.app_context():
            ...
            assert activated.lead_status == 'skip_trace'
    ...
    def test_heals_post_hold_stale_contacts_without_hold_task(self, app):
        """Mailing lead past 730d with stale contacts gets skip_trace handoff."""
        from app import db
        from app.services.skip_trace_enqueue import SkipTraceEnqueue
    ...
    def test_bounded_reconciliation_prioritizes_matured_hold_activation(self, app):
        ...
        with app.app_context():
            ...
            assert db.session.get(Lead, matured.id).lead_status == 'skip_trace'
...
class TestPromoteAwaitingSkipTraceDueLeaks:
    def test_promotes_manual_skip_trace_chore_out_of_todays_action(self, app):
        ...
        with app.app_context():
            ...
            assert lead.id in before_ids  # dated chore on skip_trace leaks into TA
    ...
    def test_reconcile_promotes_leaks_without_moving_fresh_activations(self, app):
        """Hourly reconcile promotes dated skip_trace leaks; hold activation stays skip_trace."""
        from app import db
        from app.services.skip_trace_enqueue import SkipTraceEnqueue
        ...
        with app.app_context():
            ...
            matured = _make_lead(
                app,
                '9 Hold Activation Stays Skip Trace St',
                acquisition_date=date.today() - timedelta(days=800),
            )
            SkipTraceEnqueue().schedule_recent_sale(
                matured.id,
    ...
    def test_move_already_done_preserves_future_recent_sale_hold(self, app):
        """Mid-hold must not convert the dated hold into a handoff."""
        from app import db
        from app.services.skip_trace_enqueue import SkipTraceEnqueue
        ...
        with app.app_context():
            ...
            assert hold.workflow_key == 'recent_sale_hold'
            assert hold.due_date == date.today() + timedelta(days=21)
            assert lead.needs_skip_trace is False
            assert result['already_done'] is True
            assert result['healed'] is False
            assert result['skip_trace_task_id'] == hold.id
...
class TestScheduleMailFollowUpTask:
    ...
    def test_convert_legacy_follow_up_to_rematch(self, app):
        with app.app_context():
            ...
            unrelated_mirror = Task(
                lead_id=lead.id,
                task_type='call_owner_today',
                title='Call owner — 6d Convert Rematch St',
                status='open',
                source='manual',
                due_date=datetime.combine(legacy_due, datetime.min.time()),
            )
            ...
            assert unchanged_mirror.due_date == datetime.combine(
                legacy_due,
                datetime.min.time(),
            )
    ...
    def test_convert_stale_link_does_not_claim_another_task_mirror(self, app):
        with app.app_context():
            from app import db
            from app.models import Task
            from app.services.mail_task_lifecycle_service import (
            ...
            lead = _make_lead(app, '6i Convert Protected Mirror St')
            sent_at = datetime.now(timezone.utc)
            legacy_due = sent_at.date() + timedelta(days=7)
            shared_title = 'Follow up after mailer — 6i Convert Protected Mirror St'
            protected = LeadTask(
                lead_id=lead.id,
                task_type='call_owner_today',
                title=shared_title,
            ...
            completed_stale_mirror = Task(
                lead_id=lead.id,
                task_type='call_owner_today',
                title='Follow up after mailer — completed stale link',
                status='completed',
                source='manual',
                due_date=datetime.combine(legacy_due, datetime.min.time()),
            )
            db.session.add_all([
                ...
                stale,
                protected_mirror,
                completed_stale_mirror,
            ])
            db.session.flush()
            protected.mirror_task_id = protected_mirror.id
            stale.mirror_task_id = completed_stale_mirror.id
            db.session.add_all([protected, stale])
            db.session.commit()
