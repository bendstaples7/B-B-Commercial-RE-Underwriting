...
class TestLastMailedFromMailerHistory:
    ...
    def test_ignores_silent_omit_and_voided_entries(self):
        # Distinct order ids so per-entry voided/omit filters run (not only
        # the order-level omit skip that blanks every row for one order id).
        history = [
            {
                'olc_order_id': 'ord-voided',
                'sent_at': '2026-09-10T15:34:14+00:00',
                'voided': True,
            },
            {
                'olc_order_id': 'ord-omit',
                'olc_silent_omit': True,
                'at': '2026-09-10T16:00:49+00:00',
            },
            {
                'olc_order_id': 'ord-real',
                'sent_at': '2026-09-10T15:34:14+00:00',
            },
        ]
        assert last_mailed_from_mailer_history(history) == datetime(
            2026, 9, 10, 15, 34, 14, tzinfo=timezone.utc,
        )
