...
| Skip-trace handoff | [`SkipTraceEnqueue`](../backend/app/services/skip_trace_enqueue.py) → manual `skip_trace_owner` task / `needs_skip_trace`; command-center **Move to Skip Trace** atomically completes current work, sets `lead_status=skip_trace`, and creates/reuses the handoff task | Calling a skip-trace API from EntityResolutionService; separate status/task mutations in UI; duplicate enqueue paths |
| Mail readiness | Complete owner mailing address + `recommended_action == 'mail_ready'` + [`MailQueueItem`](../backend/app/models/mail_queue_item.py) / Ready to Mail; one urgency-neutral **Direct Mail** queue action; enqueue history via [`MailEnqueueAttempt`](../backend/app/models/mail_enqueue_attempt.py); staged → pending follow-up task (`due_date` null) via [`mail_task_lifecycle_service`](../backend/app/services/mail_task_lifecycle_service.py); send sets due +7d | Property-address fallback for owner mail; competing `up_next_to_mail` as mailer status; score dampening while queued |
