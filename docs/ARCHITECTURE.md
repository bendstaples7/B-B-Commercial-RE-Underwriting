...
| Skip-trace handoff | [`SkipTraceEnqueue`](../backend/app/services/skip_trace_enqueue.py) → manual `skip_trace_owner` task / `needs_skip_trace`; `skip_trace` is the holding stage, while `awaiting_skip_trace` means verified and ready for manual skip tracing; command-center **Move to Skip Trace** handles immediate manual handoff (idempotent no-op when already in the skip-trace pipeline); recent-sale reconciliation holds the lead in `skip_trace` with no nurture action, schedules owner re-verification for the two-year mail-eligible date, and the hourly reconciler moves it to `awaiting_skip_trace` when that date arrives; **Adjust for Recent Sale** moves only the earliest open task's due date (preserving its type/title and mirrors) or creates the scheduled task when none exists | Calling a skip-trace API from EntityResolutionService; separate status/task mutations in UI; duplicate enqueue paths; treating pre-sale contacts as current; using nurture for operational holds; activating recent-sale skip trace before its hold expires |
