...
| Today's Action (due work) | [`QueueService.get_todays_action`](../backend/app/services/queue_service.py) — open **`LeadTask`** with `due_date <= today` (sorted by `lead_score`); recent-sale direct-mail work leaves only after canonical task reconciliation moves its due date; not bare `follow_up_now` | Membership via CRM `tasks` EXISTS; RA-only membership; hiding a whole recently-sold lead while unrelated work remains due; mixing proposal queues into due work |
