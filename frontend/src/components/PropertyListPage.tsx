...
/**
 * Row shape used by the AG Grid table. Extends PropertySummary with the
 * most-recent PropertyScoreRecord fields, flattened so AG Grid `valueGetter`s
 * can be avoided. `latest_score` is the raw record (or null when the property
...
const COLUMN_DEFS: ColDef<LeadRow>[] = [
  ...
  { field: 'address_2', headerName: 'Additional Address', width: 150 },
...
export const PropertyListPage: React.FC<PropertyListPageProps> = () => {
  ...
  const buildFilters = useCallback((): PropertyListFilters => {
      ...
      {activeTab === 0 && (
        <>
          ...
          <Collapse in={filtersOpen}>
            <Paper sx={{ p: { xs: 2, sm: 3 }, mb: 2 }} role="search" aria-label="Property filters">
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: '1fr 1fr 1fr' }, gap: 2 }}>
                <FormControl size="small" fullWidth>
                  <InputLabel id="filter-lead-category-label">Property Category</InputLabel>
                  <Select labelId="filter-lead-category-label" value={leadCategory} label="Property Category" onChange={(e) => setLeadCategory(e.target.value as any)}>
