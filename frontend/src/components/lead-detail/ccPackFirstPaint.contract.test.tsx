...
describe('first-paint settle — CC packing / KPI / condo pending / no_sale', () => {
  ...
  it('settles one-line address + ≤2 score drivers structure', () => {
    ...
    expect(chrome).toMatch(/ccHeaderAddressColumnSx[\s\S]{0,250}md:\s*['"]0 1 auto['"]/)
    expect(chrome).toMatch(/ccHeaderPrimaryClusterSx[\s\S]{0,250}md:\s*['"]0 1 auto['"]/)
    expect(chrome).toMatch(/ccHeaderQuickStatsSx[\s\S]{0,250}md:\s*['"]0 1 auto['"]/)
    expect(chrome).toMatch(/ccHeaderTrailingPanelsSx[\s\S]{0,250}md:\s*['"]1 1 auto['"]/)
    expect(chrome).toMatch(/clamp\(12rem,\s*16vw,\s*300px\)/)
  })
})
